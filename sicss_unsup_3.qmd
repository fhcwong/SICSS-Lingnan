...
corp_dat <- corpus(data, text_field = "text")
...
```{r}
data$ldatopics <- topics(news_lda)
```

Inspect the topic distribution

```{r}
table(data$ldatopics)
```
...
```{r}
data$document <- attributes(corp_dat)$docvar$docname_
data_topic_est <- merge(data, toipc_est, by="document")
head(data_topic_est)
```
...
```{r}
plot(effect, covariate = "source", topics = c(1), model = news_stm_cov, method = "pointestimate", main = "Effect of news source", xlim = c(-0.1, 0.5))
```

Compare topical content

```{r}
plot(news_stm_cov, type = "perspectives", topics = c(1,2))
```

Word cloud of topics

```{r}
cloud(news_stm_cov, topic = 4, scale = c(2, 0.25))
```
...
```{r}
find_k <- searchK(dfm_stm$documents, dfm_stm$vocab, K = 2:4, verbose = TRUE)
```
